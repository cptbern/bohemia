...
We assume 15 mins base time for each visit and an additional 45" for each member of the household according to Paula's estimate (11-NOV-2021, Slack). Values are in [h].

```{r}
df.workload <- df.raw.households %>%
                        mutate( visit_duration = 15/60 + n_people*45/60 ) %>% # 20" base, 15" per person ...
...
ggmap( map.moz ) +
  ...
  labs( title="Workload by Cluster (Mozambique)",
        ...
        caption="24 8h workdays, travel at 3kph, visits 15 min +45 min per person",
        x="Longitude",
        y="Latitude",
        color="Criticality") +
  guides(size=F)
